...
class PartGet(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.query = Part.objects.all().filter(active=True).order_by('-modified')
    ...
    def test_Part_view_get_one(self):
        response = self.client.get('/hierarchy/parts/{}/'.format(self.first.id))
    ...
    def test_Part_view_get_all(self, mock_date):
        mock_date.return_value = self.date
        response = self.client.get('/hierarchy/parts/')
        expected = PartSerializer(self.query, many=True).data
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, expected)

    def test_specification_view_get_filter_modify_date(self):
        response = self.client.get('/hierarchy/parts/?startdate=082516&enddate=090216')
        expected = PartSerializer(self.query.filter(modified__range=(dt(2016, 8, 25), dt(2016, 9, 2))), many=True).data
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, expected)
    ...
    def test_Part_view_get_filter_type(self, mock_date):
        mock_date.return_value = self.date
        response = self.client.get('/hierarchy/parts/?type=type1')
...
class PartModify(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Part(machine=cls.machine1, position=cls.position1, input1='one1', input2='one2', input3='one3',
             input4='one4', created=dt.now(), modified=dt.now(), type='Part1', active=True).save()
        Part(machine=cls.machine2, position=cls.position2, input1='one1', input2='one2', input3='one3',
             input4='one4', created=dt.now(), modified=dt.now(), type='Part2', active=True).save()
    ...
    def test_setpoint_view_post(self):
        ...
        response = self.client.post('/hierarchy/parts/', json.dumps(self.part),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 201)

    def test_setpoint_view_put(self):
        self.client.force_authenticate(user=self.post_modify)
        response = self.client.put('/hierarchy/parts/{}/'.format(self.update_id),
    ...
    def test_setpoint_view_delete(self):
        ...
        response = self.client.delete('/hierarchy/parts/{}/'.format(self.delete_id),
    ...
    def test_setpoint_view_delete_no_permission(self):
        self.client.force_authenticate(user=self.no_permission)
        response = self.client.put('/hierarchy/parts/{}/'.format(self.delete_id),
                                   json.dumps(self.part), content_type='application/json')
        self.assertEqual(response.status_code, 403)
...
class TestPartSerializer(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.part_malformed = {'machine': 'bad', 'position': 'bad', 'input1': 'three1', 'input2': 'three2',
                    'input3': 'three3',  'input4': 'three4', 'type': 'Part2', 'created': 'bad', 'modified': 'bad'}
        cls.part_blank = {'machine': '', 'position': '', 'input1': '', 'input2': '',
                    'input3': '',  'input4': '', 'type': '', 'modified': '', 'created': '', 'active': ''}
        cls.part_missing = {}

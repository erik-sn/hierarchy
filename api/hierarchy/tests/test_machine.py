...
class MachineGet(unittest.TestCase):
    ...
    def test_machine_view_get_filter_department(self):
        response = self.client.get('/hierarchy/machines/?department={}'.format(self.department1.id))
...
class MachineModify(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.department1 = Department.objects.get(name='department1')
        cls.department2 = Department.objects.get(name='department2')
        Machine(department=cls.department1, created=dt.now(), modified=dt.now(), name='Machine1').save()
        Machine(department=cls.department2, created=dt.now(), modified=dt.now(), name='Machine2').save()
        cls.update_id = Machine.objects.get(name='Machine1').id
        cls.delete_id = Machine.objects.get(name='Machine2').id
        cls.Machine = {'department': cls.department1.id, 'created': test_utils.get_timestamp(days=1), 'modified': test_utils.get_timestamp(),
                          'name': 'Machine3'}
    ...
    def test_setpoint_view_post(self):
        ...
        response = self.client.post('/hierarchy/machines/', json.dumps(self.Machine),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 201)

    def test_setpoint_view_put(self):
        self.client.force_authenticate(user=self.post_modify)
        response = self.client.put('/hierarchy/machines/{}/'.format(self.update_id),
    ...
    def test_setpoint_view_delete(self):
        ...
        response = self.client.delete('/hierarchy/machines/{}/'.format(self.delete_id))
    ...
    def test_setpoint_view_delete_no_permission(self):
        self.client.force_authenticate(user=self.no_permission)
        response = self.client.put('/hierarchy/machines/{}/'.format(self.delete_id),
                                   json.dumps(self.Machine), content_type='application/json')
        self.assertEqual(response.status_code, 403)
...
class TestMachineSerializer(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.Machine = {'created': test_utils.get_timestamp(days=1), 'modified': test_utils.get_timestamp(),
                          'name': 'Machine3', 'department': cls.department1.id}
        cls.Machine_malformed = {'created': 'bad', 'modified': 'bad', 'department': 'bad', 'name': 1}
        cls.Machine_blank = {'created': '', 'modified': '', 'department': '', 'name': ''}
        cls.Machine_missing = {}

...
class SiteGet(unittest.TestCase):
    ...
    def test_Site_view_get_all(self, mock_date):
        ...
        response = self.client.get('/api/v1/hierarchy/sites/')
        expected = SiteSerializer(self.query, many=True).data
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, expected)

    def test_Site_view_get_one(self):
        response = self.client.get('/api/v1/hierarchy/sites/{}/'.format(self.first.id))
        self.assertEqual(response.status_code, 200)
    ...
    def test_Site_view_get_filter_name(self, mock_date):
        mock_date.return_value = self.date
        response = self.client.get('/api/v1/hierarchy/sites/?name=name1')
...
class SiteModify(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Site(createDate=dt.now(), modifyDate=dt.now(), name='site1', code='code1', location='loc1', active=True, directory='dir1').save()
        Site(createDate=dt.now(), modifyDate=dt.now(), name='site2', code='code2', location='loc2', active=False, directory='dir2').save()
        cls.update_id = Site.objects.get(name='site1').id
        cls.delete_id = Site.objects.get(name='site2').id
        cls.site = {'createDate': test_utils.get_timestamp(days=1), 'modifyDate': test_utils.get_timestamp(),
    ...
    def test_setpoint_view_post(self):
        ...
        response = self.client.post('/api/v1/hierarchy/sites/', json.dumps(self.site),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 201)

    def test_setpoint_view_put(self):
        self.client.force_authenticate(user=self.post_modify)
        response = self.client.put('/api/v1/hierarchy/sites/{}/'.format(self.update_id),
    ...
    def test_setpoint_view_delete(self):
        ...
        response = self.client.delete('/api/v1/hierarchy/sites/{}/'.format(self.delete_id))
    ...
    def test_setpoint_view_delete_no_permission(self):
        self.client.force_authenticate(user=self.no_permission)
        response = self.client.put('/api/v1/hierarchy/sites/{}/'.format(self.delete_id),
                                   json.dumps(self.site), content_type='application/json')
        self.assertEqual(response.status_code, 403)
...
class TestSiteSerializer(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.site = {'createDate': test_utils.get_timestamp(days=1), 'modifyDate': test_utils.get_timestamp(),
                         'name': 'site3', 'code': 'code3', 'location': 'loc3', 'active': True, 'directory': 'dir3'}
        cls.site_malformed = {'createDate': 'bad', 'modifyDate': 'bad', 'name': 'site3', 'code': 'code3',
                                   'location': 'loc3', 'active': 'bad', 'directory': 'dir3'}
        cls.site_blank = {'createDate': '', 'modifyDate': '', 'name': '', 'code': '',
                                   'location': '', 'active': '', 'directory': ''}
        cls.site_missing = {}

        cls.malformed_checks = ['createDate', 'modifyDate']
        cls.blank_checks = ['name', 'code', 'location', 'directory', 'createDate', 'modifyDate']
        cls.missing_checks = ['name', 'code', 'location', 'directory']

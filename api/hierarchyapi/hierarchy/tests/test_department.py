...
class DepartmentGet(unittest.TestCase):
    ...
    def test_Department_view_get_filter_id(self):
        response = self.client.get('/api/v1/hierarchy/departments/?id={}'.format(self.first.id))
...
class DepartmentModify(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.site1 = Site.objects.get(name='site1')
        cls.site2 = Site.objects.get(name='site2')
        Department(site=cls.site1, createDate=dt.now(), modifyDate=dt.now(), name='Department1', active=True).save()
        Department(site=cls.site2, createDate=dt.now(), modifyDate=dt.now(), name='Department2', active=False).save()
        cls.update_id = Department.objects.get(name='Department1').id
        cls.delete_id = Department.objects.get(name='Department2').id
        print(cls.delete_id)
        cls.department = {'site': cls.site1.id, 'createDate': test_utils.get_timestamp(days=1), 'modifyDate': test_utils.get_timestamp(),
                          'name': 'Department3'}
    ...
    def test_setpoint_view_post(self):
        ...
        response = self.client.post('/api/v1/hierarchy/departments/', json.dumps(self.department),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 201)

    def test_setpoint_view_put(self):
        self.client.force_authenticate(user=self.post_modify)
        response = self.client.put('/api/v1/hierarchy/departments/{}/'.format(self.update_id),
    ...
    def test_setpoint_view_delete(self):
        ...
        response = self.client.delete('/api/v1/hierarchy/departments/{}/'.format(self.delete_id))
    ...
    def test_setpoint_view_delete_no_permission(self):
        self.client.force_authenticate(user=self.no_permission)
        response = self.client.put('/api/v1/hierarchy/departments/{}/'.format(self.delete_id),
                                   json.dumps(self.department), content_type='application/json')
        self.assertEqual(response.status_code, 403)
...
class TestDepartmentSerializer(unittest.TestCase):
    ...
    def setUpClass(cls):
        Site(name='site1').save()
        cls.site1 = Site.objects.get(name='site1')
        cls.department = {'createDate': test_utils.get_timestamp(days=1), 'modifyDate': test_utils.get_timestamp(),
                          'name': 'Department3', 'site': cls.site1.id}
        cls.department_malformed = {'createDate': 'bad', 'modifyDate': 'bad', 'name': 'Department3', 'code': 'code3',
                                    'location': 'loc3', 'directory': 'dir3', 'site': 'bad'}
        cls.department_blank = {'createDate': '', 'modifyDate': '', 'name': '', 'directory': '', 'site': ''}
        cls.department_missing = {}
        cls.malformed_checks = ['createDate', 'modifyDate', 'site']
        cls.blank_checks = ['createDate', 'modifyDate', 'name', 'site']
